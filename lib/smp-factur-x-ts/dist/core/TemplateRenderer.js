...
class TemplateRenderer {
    ...
    async attachFacturXml() {
        ...
        try {
            ...
            const names = catalog.lookup(pdf_lib_1.PDFName.of('Names'), pdf_lib_1.PDFDict);
            if (names) {
                const embeddedFiles = names.lookup(pdf_lib_1.PDFName.of('EmbeddedFiles'), pdf_lib_1.PDFDict);
                if (embeddedFiles) {
                    const namesKey = pdf_lib_1.PDFName.of('Names');
                    const nameArrayRef = embeddedFiles.get(namesKey);
                    if (nameArrayRef) {
                        const nameArray = this.pdfDoc.context.lookup(nameArrayRef);
                        if (nameArray && nameArray.length >= 2) {
                            const fileSpecRef = nameArray[nameArray.length - 1];
                            const fileSpec = this.pdfDoc.context.lookup(fileSpecRef, pdf_lib_1.PDFDict);
                            if (fileSpec) {
                                (0, PDFA3Compliance_1.addAFRelationshipToFile)(fileSpec, 'Data');
                            }
                        }
                    }
                }
