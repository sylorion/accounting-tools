...
class TemplateRenderer {
    ...
    async drawContinuationPageHeaders() {
        ...
        for (let i = 1; i < this.allPages.length; i++) {
            ...
            page.drawText(`${this.strings.issueDate}: ${this.formatInvoiceDateFull()}`, { x: sx, y: topY - 32, size: 8, font, color: this.parseColor('#64748b') });
            page.drawText(`${this.strings.grandTotal}: ${(0, core_1.formatAmount)(summary.grandTotal)} ${invoice.currency}`, { x: sx, y: topY - 46, size: 9, font: fontBold, color: this.parseColor('#1e293b') });
            page.drawText(`${this.strings.dueDate}: ${this.formatDateFull(this.getDueDate())}`, { x: sx, y: topY - 60, size: 8, font, color: this.parseColor('#64748b') });
