...
class TemplateRenderer {
    ...
    async drawContinuationPageHeaders() {
        ...
        for (let i = 1; i < this.allPages.length; i++) {
            ...
            page.drawText(`${this.strings.issueDate}: ${this.formatInvoiceDateFull()}`, { x: cx, y: topY - 30, size: 7.5, font, color: this.parseColor('#64748b') });
            page.drawText(`${this.strings.grandTotal}: ${(0, core_1.formatAmount)(summary.grandTotal)} ${invoice.currency}`, { x: cx, y: topY - 42, size: 8.5, font: fontBold, color: this.parseColor('#1e293b') });
            page.drawText(`${this.strings.dueDate}: ${this.formatDateFull(this.getDueDate())}`, { x: cx, y: topY - 55, size: 7.5, font, color: this.parseColor('#64748b') });
            if (qrImage) {
